...
public:
    ...
    void fordFulkerson(int s, int t)
    {
        ...
        while (bfs(rdata, s, t, parent))
        {
            ...
            maxFlow += pathFlow;
            //cout << "Man " <<parent[parent[t]] << " Woman " << parent[t]<< endl;
            pairs.push({id[parent[parent[t]]],id[parent[t]]});
            //  cout << "Man " <<id[parent[parent[t]]] << " Woman " << id[parent[t]]<< endl;
        }
...
int main()
{
    ...
    for (int i = 1; i <= n + m; i++)
    {
        ...
        cin >> _conditions[i].height >> _conditions[i].age >> g.id[i];
    }
    for (int i = 1; i <= n; i++)
    {
        for (int j = n + 1; j <= n + m; j++)
        {
            if ((abs(_conditions[i].height - _conditions[j].height) <= 10) and
                (abs(_conditions[i].age - _conditions[j].age) <= 5))
            {
    ...
    while(!pairs.empty()){
        int x=pairs.top().first;
        int y=pairs.top().second;
        cout << "Man " <<x << "- Woman " << y<< endl;
        pairs.pop();
    }
}